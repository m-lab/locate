...
// extractJWTClaims extracts JWT claims using the configured JWT verifier.
// The verifier mode is determined by the --jwt-auth-mode flag.
func (c *Client) extractJWTClaims(req *http.Request) (map[string]interface{}, error) {
	claims, err := c.jwtVerifier.ExtractClaims(req)
	if err != nil {
		return nil, err
	}

	log.WithFields(log.Fields{
		"mode": c.jwtVerifier.Mode(),
	}).Debug("JWT claims extracted")
